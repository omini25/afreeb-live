...
function Login() {
    ...
    const handleGoogleLogin = async (event) => {
        ...
        try {
            // Replace with your actual Google API endpoint and necessary parameters
            const response = await axios.get(`${server}/auth/google`);

            if (response.status === 200) {
                const data = await response.data;
                // Handle successful login response from Google API
                // For example, store user data in local storage or redirect to a protected page
